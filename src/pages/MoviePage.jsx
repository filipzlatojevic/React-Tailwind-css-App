...
const MoviePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchClips = async () => {
      try {
        const response = await MoviesService.getClips(movieId);
        setClips(response);
      } catch (e) {
        console.log('ERROR: ', e);
        alert(e);
  ...
  return (
    <div className="bg-gray-700 min-h-[100vh] text-white text-3xl sm:text-4xl md:text-3xl lg:text-4xl xl:text-6xl font-bold">
      ...
      />
      <div className="flex flex-col items-center min-[2200px]:mt-[50%] mt-[45%] justify-center md:flex-row md:ml-[50px]">
        <img
      ...
      <div className="mt-5 md:mt-10 text-xl md:text-2xl lg:text-4xl pb-[100px] mx-2 sm:mx-5 md:mx-[50px] lg:mx-[100px]">
          ...
          {clips.map(clip => (
            <div
              ...
              <div className="relative flex-shrink-0 h-[180px] md:h-[250px] lg:h-[300px] aspect-video rounded-xl">
                ...
                />
                <img src={play_icon} alt="play icon" className="absolute inset-0 w-[150px] h-[150px] m-auto" />
              </div>
        ...
        <div className="mt-5 md:mt-10 font-normal text-lg md:text-xl lg:text-2xl">{movie.overview}</div>
      </div>
    </div>
  );
};
